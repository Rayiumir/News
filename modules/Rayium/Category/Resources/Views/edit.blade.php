            ...
            <div class="card-body">
                <form action="{{ route('category.update', $category->id) }}" method="POST">
                    ...
                    <div class="mb-3">
                        ...
                        <input type="text" name="title" class="form-control rounded-5 @error('title') is-invalid @enderror" id="Input1" value="{{ $category->title }}">
                        @error('title')
                    ...
                    <div class="mb-3">
                        ...
                        <input type="text" name="keywords" class="form-control rounded-5 @error('keywords') is-invalid @enderror" id="Input2" {{ $category->keywords }}>
                        @error('keywords')
                        <span class="invalid-feedback" role="alert">
                                 <strong>{{ $message }}</strong>
                            </span>
                    ...
                    <div class="mb-3">
                        ...
                        <textarea rows="3" name="description" class="form-control rounded-5 @error('description') is-invalid @enderror" id="Input3" {{ $category->description }}></textarea>
                        @error('description')
                        <span class="invalid-feedback" role="alert">
                                 <strong>{{ $message }}</strong>
                            </span>
                    ...
                    <div class="mb-3">
                        ...
                        <select type="text" name="status" class="form-select rounded-5 @error('status') is-invalid @enderror" id="Input4">
                            ...
                            @foreach(\modules\Rayium\Category\Models\Category::$statuses as $row)
                                <option @if($category->status === $row) selected @endif value="{{$row}}">@lang($row)</option>
                            @endforeach
                        </select>
                        @error('status')
                        <span class="invalid-feedback" role="alert">
